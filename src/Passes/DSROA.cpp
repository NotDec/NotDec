...
long NumLoadsSpeculated = 0;
long NumVectorized = 0;
...
public:
  AllocaSliceRewriter(const DataLayout &DL, AllocaSlices &AS, DSROAPass &Pass,
...
using dsroa::AllocaSliceRewriter;
using dsroa::AllocaSlices;
using dsroa::Partition;
using dsroa::Slice;

PreservedAnalyses DSROAPass::run(Function &F, FunctionAnalysisManager &AM) {
  LLVM_DEBUG(dbgs() << "DSROA function: " << F.getName() << "\n");
  C = &F.getContext();
...
/// rewritten as needed.
bool DSROAPass::runOnAlloca(AllocaInst &AI) {
  LLVM_DEBUG(dbgs() << "DSROA alloca: " << AI << "\n");
  ++NumAllocasAnalyzed;
  ...
  while (!SpeculatablePHIs.empty())
    dsroa::speculatePHINodeLoads(IRB, *SpeculatablePHIs.pop_back_val());

  LLVM_DEBUG(dbgs() << "  Speculating Selects\n");
  while (!SpeculatableSelects.empty())
    dsroa::speculateSelectInstLoads(IRB, *SpeculatableSelects.pop_back_val());
...
bool DSROAPass::deleteDeadInstructions(
...
bool DSROAPass::splitAlloca(AllocaInst &AI, AllocaSlices &AS) {
  if (AS.begin() == AS.end())
    return false;
  ...
  MaxPartitionsPerAlloca = std::max(MaxPartitionsPerAlloca, NumPartitions);
...
bool DSROAPass::presplitLoadsAndStores(AllocaInst &AI, AllocaSlices &AS) {
...
/// promoted.
AllocaInst *DSROAPass::rewritePartition(AllocaInst &AI, AllocaSlices &AS,
                                        Partition &P) {
  ...
  MaxUsesPerAllocaPartition = std::max(MaxUsesPerAllocaPartition, NumUses);

  // Now that we've processed all the slices in the new partition, check if any
  // PHIs or Selects would block promotion.
  for (PHINode *PHI : PHIUsers)
    if (!dsroa::isSafePHIToSpeculate(*PHI)) {
      Promotable = false;
      PHIUsers.clear();
      SelectUsers.clear();
      break;
  ...
  for (SelectInst *Sel : SelectUsers)
    if (!dsroa::isSafeSelectToSpeculate(*Sel)) {
      Promotable = false;
      PHIUsers.clear();
      SelectUsers.clear();
      break;
...
void DSROAPass::clobberUse(Use &U) {
  Value *OldV = U;
...
bool DSROAPass::promoteAllocas(Function &F) {
