...
- `include/notdec/Passes/DangerousTypePatternScan.h`：pass 声明与设计说明。
- `src/Passes/DangerousTypePatternScan.cpp`：
  - `stripToStructType`：把 GEP source element type 剥到最内层 struct（支持 `[N x struct]`）。
  - `isDynamicArrayIndex`：最后索引非常量 0 视为动态数组访问。
  - `collectPoolReturners`：找"返回 S* 的函数"（GEP(S) 结果经 phi/bitcast/select 传播出现在 return）。
  - `hasPoolLinkPattern`：调用返回 S* 后把返回值 store 到 `GEP(i8, load, off)`（挂到 load 出来的对象字段）→ 判定为递归近似。
  - `run`：统计 (函数, struct) 的动态数组 GEP 次数 × DataLayout struct 大小 × weight（递归 3 / 非递归 1）；递归命中且 score≥阈值（默认 10000，`NOTDEC_DANGEROUS_SCAN_THRESHOLD` 可调）报 HIGH warning；无递归证据但 score≥10 倍阈值报 Note。
- `src/Passes/PassManager.cpp`：`add_type_recovery_passes` 里 `MLsubRecoveryMain` 前插入 pass（约 468 行）。
- `src/CMakeLists.txt`：源文件列表加入新 cpp。

验证（build-relwithdebinfo-20260731/bin/notdec）：

- memcached 全量 401：只报 `resp_allocate`（score=46176，_mc_resp geps=13 size=1184 weight=3），与实测爆炸函数一致。
- 单函数 resp_allocate（260b 上下文无 resp_start）：score=15392 且无递归证据，不报 warning，与"单函数不爆"一致。
- 260c（resp_start+resp_allocate）：报 HIGH，递归近似命中。
- vsftpd / libuv 全量：无 warning，pass 无副作用。

已知限制：递归检测是无 DebugInfo 的值流近似，只覆盖"池分配返回对象挂入结构"模式；其他自引用形态（如直接 struct GEP 字段 store S* 值）未覆盖，可能漏报。warning 是提示性质，阈值可按项目调。
