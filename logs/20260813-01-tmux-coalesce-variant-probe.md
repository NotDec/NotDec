...
## 追加：顺序影响性能的机制与启发（2026-08-14 第五轮）

关键实验：当前二进制（确定性修复后）开 ASLR 全量再跑一次，21:55 完成、
同样 0 个大 group——修复后的固定顺序本身落在“无大组”结构上，抽奖消失，
与 ASLR 无关。

分组与性能的对照表（全量 tmux）：

| 代码版本 | 大组 recvars | 最大 go_calls | 墙钟 |
| --- | --- | ---: | ---: |
| v1 缓存 / opt2b / 交集微优化 | 484-862 | 1.4-1.6M | 16-48min |
| analyze 缓存 | 295/300/307/791 | 600k | 16:27/17:02（两次相同） |
| analyze 缓存 + 列表共享 | 143 | 30-40M | 34-49min 被杀 |
| 确定性修复后 | 无大组 | 0 | 20:37/21:55（两次） |

机制：顺序决定两类决策，决策决定递归结构，递归结构决定指数级展开成本。

1. merge 决策：先合并哪对变量由约束插入/求解顺序决定。合并把两个位置的
   bounds 绑在一起，不同合并集 → 不同的互递归嵌套结构。
2. 折叠决策：canonicalize 的 `AlreadyRecursive` 先到先得——谁先展开某个
   key 谁决定它是否成为递归变量；并行 canonicalize 下这是竞态，串行下是
   顺序依赖。
3. 成本不是“递归变量越多越慢”：143 个 recvars 的组展开 4000 万次 go，
   791 个的组只有 60 万。爆炸来自深层嵌套互递归的路径组合（coalesce 按
   引用路径重展开），与变量数量非单调。

启发：

- 折叠/合并决策应内容决定而非顺序决定（按结构签名决定是否折叠，替代查表
  先到先得），这样结构稳定且可以主动选择“好”结构。
- merge 时机值得系统对比：项目已有 early-merge 对照开关，延迟到约束收敛后
  再 merge 通常避免基于不完整信息的过度合并。
- 折叠密度（20260808-02 探针）可预测爆炸，可对高密度组提前采用有界展开或
  折叠 key 等价类合并。
- 确定性排序的价值：可复现后才能做结构级 A/B；本次固定顺序恰好落在无大组
  结构，但换一个固定顺序可能落在坏结构上，需要按上述内容级决策根治。

## 追加：merge 时机正式对照（2026-08-14 第六轮）

`NOTDEC_EARLY_CALL_INTERFACE_MERGE=1`（在加调用边之前记录并应用 merge 候选）
对全量 tmux 的对照：

| 配置 | 非零 merge 次数 | 合并总对数 | 墙钟 | 峰值 RSS | 大组 |
| --- | ---: | ---: | ---: | ---: | ---: |
| late（默认，边加完再合） | 10 | 5466（最大 4328） | 20:37 / 21:55 | 13.7-15.2GB | 0 |
| early（边加前合） | 2 | 1046（最大 597） | 52:56 未完成被杀 | 19.8GB | 32 行 |

机制：call-interface 候选需要 actual/formal 边已存在才能记录，early 模式下
信息不完整 → call-interface 合并为 0 对；类型图保持细碎复杂 → 大组重新出现
（32 行 simplify-expanding）→ 墙钟翻倍以上。

结论：merge 的作用是“把等价调用槽合并、扁平化类型图”，合并越多（正确时）
结构越平、越快；时机必须是信息完整之后。当前默认 late merge 是对的；进一步
提速方向是“更精确地合并更多”（现有 8 个 bad_unions 是过度合并，但方向仍是
更准而不是更少），以及内容决定的折叠决策。
