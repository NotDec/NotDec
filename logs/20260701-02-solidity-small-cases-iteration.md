...
## 2026-07-01：return bool not eq zero uint

问题：

`return_bool_not_eq_zero_uint_01` 的源码语义是 `return !(value == 0);`。冻结 IR 使用 `icmp eq value, 0` 后接 `xor i1 true`，用于确认 bool 逻辑取反不会错误打印成按位 `~`。当前输出会归一化为等价的 `value != 0`。

改动：

- [test/evm/solidity-source/cases/return_bool_not_eq_zero_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/cases/return_bool_not_eq_zero_uint_01.sol:1) 新增源码证据，源码第 5 行函数 `notzero()` 在第 6 行返回 `!(value == 0)`。
- [test/evm/solidity-source/ir/return_bool_not_eq_zero_uint_01.ll](/sn640/NotDec/test/evm/solidity-source/ir/return_bool_not_eq_zero_uint_01.ll:8) 新增冻结 IR，`public_notzero_uint256__0x2a()` 第 10 行生成 `icmp eq i256 %arg0, 0`，第 11 行用 `xor i1 true` 表示 bool 取反，第 12 行 `zext i1` 成 ABI word。
- [test/evm/solidity-source/expected/return_bool_not_eq_zero_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/expected/return_bool_not_eq_zero_uint_01.sol:1) 固化输出，函数返回类型为 `bool`，第 4 行是归一化后的 `return arg0 != 0;`。
...
```bash
./llvm-22.1.0.obj/bin/llvm-as test/evm/solidity-source/ir/return_bool_not_eq_zero_uint_01.ll -o /tmp/return_bool_not_eq_zero_uint_01.bc
./build/bin/notdec test/evm/solidity-source/ir/return_bool_not_eq_zero_uint_01.ll -o /tmp/return_bool_not_eq_zero_uint_01.before.sol --tr-level=2
ctest --test-dir build -R notdec.evm.solidity_source --output-on-failure
/usr/bin/time -f 'elapsed=%e user=%U sys=%S maxrss=%M' ./build/bin/notdec test/evm/solidity-patterns/cases/25928_19774281_d048a8d52d_2758caa02f46.ll -o /tmp/notdec-solidity-bool-not-eq-zero-smoke.sol --tr-level=2
```

结果：

`return_bool_not_eq_zero_uint_01` 当前输出：

```solidity
contract Decompiled {
    function notzero(uint256 arg0) public returns (bool ret0) {
        // block_0:
        return arg0 != 0;
    }
}
...
`notdec.evm.solidity_source` 通过，用时 5.88 秒。

性能：

EVM smoke 用时 `elapsed=30.17 user=34.25 sys=0.74 maxrss=971172`。

## 2026-07-01：return bool sgt int

问题：

`return_bool_sgt_int_01` 的源码语义是 `return left > right;`，参数类型是 `int256`。前面已经固化了 signed `<`，这里补 signed `>`，确认 `icmp sgt` 和 ABI `int256` 参数能一起打印成 Solidity signed 比较。

改动：

- [test/evm/solidity-source/cases/return_bool_sgt_int_01.sol](/sn640/NotDec/test/evm/solidity-source/cases/return_bool_sgt_int_01.sol:1) 新增源码证据，源码第 5 行函数 `issgt()` 使用 `int256` 参数，第 6 行返回 `left > right`。
- [test/evm/solidity-source/ir/return_bool_sgt_int_01.ll](/sn640/NotDec/test/evm/solidity-source/ir/return_bool_sgt_int_01.ll:8) 新增冻结 IR，`public_issgt_int256_int256__0x2a()` 第 10 行生成 `icmp sgt`，第 11 行 `zext i1` 成 ABI word。
- [test/evm/solidity-source/expected/return_bool_sgt_int_01.sol](/sn640/NotDec/test/evm/solidity-source/expected/return_bool_sgt_int_01.sol:1) 固化输出，函数参数类型为 `int256`，返回类型为 `bool`，第 4 行是 `return arg0 > arg1;`。
...
```bash
./llvm-22.1.0.obj/bin/llvm-as test/evm/solidity-source/ir/return_bool_sgt_int_01.ll -o /tmp/return_bool_sgt_int_01.bc
./build/bin/notdec test/evm/solidity-source/ir/return_bool_sgt_int_01.ll -o /tmp/return_bool_sgt_int_01.after.sol --tr-level=2
ctest --test-dir build -R notdec.evm.solidity_source --output-on-failure
/usr/bin/time -f 'elapsed=%e user=%U sys=%S maxrss=%M' ./build/bin/notdec test/evm/solidity-patterns/cases/25928_19774281_d048a8d52d_2758caa02f46.ll -o /tmp/notdec-solidity-bool-sgt-smoke.sol --tr-level=2
```

结果：

`return_bool_sgt_int_01` 当前输出：
...
## 2026-07-01：return bool sge int

问题：

`return_bool_sge_int_01` 的源码语义是 `return left >= right;`，参数类型是 `int256`。前面已经固化了 signed `<`、`>` 和 `<=`，这里补 signed `>=`，确认 `icmp sge` 和 ABI `int256` 参数能一起打印成 Solidity signed 比较。

改动：

- [test/evm/solidity-source/cases/return_bool_sge_int_01.sol](/sn640/NotDec/test/evm/solidity-source/cases/return_bool_sge_int_01.sol:1) 新增源码证据，源码第 5 行函数 `issge()` 使用 `int256` 参数，第 6 行返回 `left >= right`。
- [test/evm/solidity-source/ir/return_bool_sge_int_01.ll](/sn640/NotDec/test/evm/solidity-source/ir/return_bool_sge_int_01.ll:8) 新增冻结 IR，`public_issge_int256_int256__0x2a()` 第 10 行生成 `icmp sge`，第 11 行 `zext i1` 成 ABI word。
- [test/evm/solidity-source/expected/return_bool_sge_int_01.sol](/sn640/NotDec/test/evm/solidity-source/expected/return_bool_sge_int_01.sol:1) 固化输出，函数参数类型为 `int256`，返回类型为 `bool`，第 4 行是 `return arg0 >= arg1;`。
- [test/evm/solidity-source/manifest.json](/sn640/NotDec/test/evm/solidity-source/manifest.json:309) 把新 case 接入 `notdec.evm.solidity_source`。

验证：

```bash
./llvm-22.1.0.obj/bin/llvm-as test/evm/solidity-source/ir/return_bool_sge_int_01.ll -o /tmp/return_bool_sge_int_01.bc
./build/bin/notdec test/evm/solidity-source/ir/return_bool_sge_int_01.ll -o /tmp/return_bool_sge_int_01.after.sol --tr-level=2
ctest --test-dir build -R notdec.evm.solidity_source --output-on-failure
/usr/bin/time -f 'elapsed=%e user=%U sys=%S maxrss=%M' ./build/bin/notdec test/evm/solidity-patterns/cases/25928_19774281_d048a8d52d_2758caa02f46.ll -o /tmp/notdec-solidity-bool-sge-smoke.sol --tr-level=2
```

结果：

`return_bool_sge_int_01` 当前输出：

```solidity
contract Decompiled {
    function issge(int256 arg0, int256 arg1) public returns (bool ret0) {
        // block_0:
        return arg0 >= arg1;
    }
}
...
`notdec.evm.solidity_source` 通过，用时 6.19 秒。
...
## 2026-07-01：return bool or eq zero uint

问题：

`return_bool_or_eq_zero_uint_01` 的源码语义是 `return left == 0 || right == 0;`。修复前当前输出是 `return (arg0 == 0) | (arg1 == 0);`，把 `i1 or` 当成了位运算打印。

改动：

- [external/NotDec-llvm2c/lib/Solidity/BodyBuilder.cpp](/sn640/NotDec/external/NotDec-llvm2c/lib/Solidity/BodyBuilder.cpp:334) 新增 `logicalOperatorText()`，只把 LLVM `and/or` 的 bool 形态映射为 Solidity `&&/||`。
- [external/NotDec-llvm2c/lib/Solidity/BodyBuilder.cpp](/sn640/NotDec/external/NotDec-llvm2c/lib/Solidity/BodyBuilder.cpp:379) 新增 `logicalOperatorPrecedence()`，补 `||` 和 `&&` 的优先级。
- [external/NotDec-llvm2c/lib/Solidity/BodyBuilder.cpp](/sn640/NotDec/external/NotDec-llvm2c/lib/Solidity/BodyBuilder.cpp:544) 在 `formatReturnValue()` 中优先识别 `i1 and/or`，普通 `i256` 位运算仍走原来的 `&/|` 打印。
- [test/evm/solidity-source/cases/return_bool_or_eq_zero_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/cases/return_bool_or_eq_zero_uint_01.sol:1) 新增源码证据，源码第 5 行函数 `eitherzero()` 使用两个 `uint256` 参数，第 6 行返回两个零比较的 `||`。
- [test/evm/solidity-source/ir/return_bool_or_eq_zero_uint_01.ll](/sn640/NotDec/test/evm/solidity-source/ir/return_bool_or_eq_zero_uint_01.ll:8) 新增冻结 IR，`public_eitherzero_uint256_uint256__0x2a()` 第 10-13 行用两个 `icmp eq`、一个 `or i1` 和 `zext` 表示 bool 返回。
- [test/evm/solidity-source/expected/return_bool_or_eq_zero_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/expected/return_bool_or_eq_zero_uint_01.sol:1) 固化输出，第 4 行为 `return arg0 == 0 || arg1 == 0;`。
- [test/evm/solidity-source/manifest.json](/sn640/NotDec/test/evm/solidity-source/manifest.json:303) 把新 case 接入 `notdec.evm.solidity_source`。
...
```bash
cmake --build ./build --target notdec -j4
./llvm-22.1.0.obj/bin/llvm-as test/evm/solidity-source/ir/return_bool_or_eq_zero_uint_01.ll -o /tmp/return_bool_or_eq_zero_uint_01.bc
./build/bin/notdec test/evm/solidity-source/ir/return_bool_or_eq_zero_uint_01.ll -o /tmp/return_bool_or_eq_zero_uint_01.after.sol --tr-level=2
ctest --test-dir build -R notdec.evm.solidity_source --output-on-failure
ctest --test-dir build -R notdec.evm.solidity_rewrite --output-on-failure
/usr/bin/time -f 'elapsed=%e user=%U sys=%S maxrss=%M' ./build/bin/notdec test/evm/solidity-patterns/cases/25928_19774281_d048a8d52d_2758caa02f46.ll -o /tmp/notdec-solidity-bool-or-eq-zero-smoke.sol --tr-level=2
```

结果：

`return_bool_or_eq_zero_uint_01` 当前输出：

```solidity
contract Decompiled {
    function eitherzero(uint256 arg0, uint256 arg1) public returns (bool ret0) {
        // block_0:
        return arg0 == 0 || arg1 == 0;
    }
}
...
`notdec.evm.solidity_source` 通过，用时 6.48 秒。`notdec.evm.solidity_rewrite` 通过，用时 100.22 秒。

性能：

EVM smoke 用时 `elapsed=29.95 user=34.10 sys=0.70 maxrss=981488`。
...
## 2026-07-01：return bool or ne zero uint

问题：

`return_bool_or_ne_zero_uint_01` 的源码语义是 `return left != 0 || right != 0;`。修复前当前输出是 `return arg0 | arg1 != 0;`，语义接近但不够像 Solidity 源码。

改动：

- [external/NotDec-llvm2c/lib/Solidity/BodyBuilder.cpp](/sn640/NotDec/external/NotDec-llvm2c/lib/Solidity/BodyBuilder.cpp:493) 扩展 `formatReturnValue()` 里的 `icmp eq/ne (or x, y), 0` pattern，`== 0` 打印为两个零比较的 `&&`，`!= 0` 打印为两个非零比较的 `||`。
- [test/evm/solidity-source/cases/return_bool_or_ne_zero_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/cases/return_bool_or_ne_zero_uint_01.sol:1) 新增源码证据，源码第 5 行函数 `eithernonzero()` 使用两个 `uint256` 参数，第 6 行返回两个非零比较的 `||`。
- [test/evm/solidity-source/ir/return_bool_or_ne_zero_uint_01.ll](/sn640/NotDec/test/evm/solidity-source/ir/return_bool_or_ne_zero_uint_01.ll:8) 新增冻结 IR，`public_eithernonzero_uint256_uint256__0x2a()` 第 10-13 行用两个 `icmp ne`、一个 `or i1` 和 `zext` 表示 bool 返回。
- [test/evm/solidity-source/expected/return_bool_or_ne_zero_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/expected/return_bool_or_ne_zero_uint_01.sol:1) 固化输出，第 4 行为 `return arg0 != 0 || arg1 != 0;`。
- [test/evm/solidity-source/manifest.json](/sn640/NotDec/test/evm/solidity-source/manifest.json:297) 把新 case 接入 `notdec.evm.solidity_source`。
...
```bash
cmake --build ./build --target notdec -j4
./llvm-22.1.0.obj/bin/llvm-as test/evm/solidity-source/ir/return_bool_or_ne_zero_uint_01.ll -o /tmp/return_bool_or_ne_zero_uint_01.bc
./build/bin/notdec test/evm/solidity-source/ir/return_bool_or_ne_zero_uint_01.ll -o /tmp/return_bool_or_ne_zero_uint_01.fixed.sol --tr-level=2
./build/bin/notdec test/evm/solidity-source/ir/return_bool_and_eq_zero_uint_01.ll -o /tmp/return_bool_and_eq_zero_uint_01.regress.sol --tr-level=2
ctest --test-dir build -R notdec.evm.solidity_source --output-on-failure
ctest --test-dir build -R notdec.evm.solidity_rewrite --output-on-failure
/usr/bin/time -f 'elapsed=%e user=%U sys=%S maxrss=%M' ./build/bin/notdec test/evm/solidity-patterns/cases/25928_19774281_d048a8d52d_2758caa02f46.ll -o /tmp/notdec-solidity-bool-or-ne-zero-smoke.sol --tr-level=2
```

结果：

`return_bool_or_ne_zero_uint_01` 当前输出：

```solidity
contract Decompiled {
    function eithernonzero(uint256 arg0, uint256 arg1) public returns (bool ret0) {
        // block_0:
        return arg0 != 0 || arg1 != 0;
    }
}
...
`return_bool_and_eq_zero_uint_01` 回归输出仍为 `return arg0 == 0 && arg1 == 0;`。

`notdec.evm.solidity_source` 通过，用时 6.68 秒。`notdec.evm.solidity_rewrite` 通过，用时 100.90 秒。

性能：

EVM smoke 用时 `elapsed=29.86 user=34.02 sys=0.69 maxrss=983756`。

评估：

效果：把 `OR` 后与零做非等比较的常见 bool 形态打印回 Solidity `||`。复杂度：低，复用已有 `OR == 0` pattern，只补 `!= 0` 分支。维护成本：低；但这个仍是局部 pattern，复杂 bool 表达式后续应继续往独立 Solidity 表达式 AST 收敛。

## 2026-07-01：return bool range uint

问题：

`return_bool_range_uint_01` 的源码语义是 `return lower < value && value < upper;`。这是对多个无符号比较通过 `i1 and` 组合时的打印回归，确认比较优先级和 Solidity `&&` 输出稳定。

改动：

- [test/evm/solidity-source/cases/return_bool_range_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/cases/return_bool_range_uint_01.sol:1) 新增源码证据，源码第 5 行函数 `inrange()` 使用三个 `uint256` 参数，第 6 行返回两个 `<` 比较的 `&&`。
- [test/evm/solidity-source/ir/return_bool_range_uint_01.ll](/sn640/NotDec/test/evm/solidity-source/ir/return_bool_range_uint_01.ll:8) 新增冻结 IR，`public_inrange_uint256_uint256_uint256__0x2a()` 第 10-13 行用两个 `icmp ult`、一个 `and i1` 和 `zext` 表示 bool 返回。
- [test/evm/solidity-source/expected/return_bool_range_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/expected/return_bool_range_uint_01.sol:1) 固化输出，第 4 行为 `return arg1 < arg0 && arg0 < arg2;`。
- [test/evm/solidity-source/manifest.json](/sn640/NotDec/test/evm/solidity-source/manifest.json:279) 把新 case 接入 `notdec.evm.solidity_source`。

验证：

```bash
./llvm-22.1.0.obj/bin/llvm-as test/evm/solidity-source/ir/return_bool_range_uint_01.ll -o /tmp/return_bool_range_uint_01.bc
./build/bin/notdec test/evm/solidity-source/ir/return_bool_range_uint_01.ll -o /tmp/return_bool_range_uint_01.after.sol --tr-level=2
ctest --test-dir build -R notdec.evm.solidity_source --output-on-failure
/usr/bin/time -f 'elapsed=%e user=%U sys=%S maxrss=%M' ./build/bin/notdec test/evm/solidity-patterns/cases/25928_19774281_d048a8d52d_2758caa02f46.ll -o /tmp/notdec-solidity-bool-range-smoke.sol --tr-level=2
```

结果：

`return_bool_range_uint_01` 当前输出：

```solidity
contract Decompiled {
    function inrange(uint256 arg0, uint256 arg1, uint256 arg2) public returns (bool ret0) {
        // block_0:
...
`notdec.evm.solidity_source` 通过，用时 6.83 秒。
...
## 2026-07-01：return bool and or lt uint

问题：

`return_bool_and_or_lt_uint_01` 的源码语义是 `return a < b && (c < d || e < f);`。这是对 `&&` 右侧嵌套 `||` 时括号保留的打印回归，如果少了括号，Solidity 语义会变成 `(a < b && c < d) || e < f`。

改动：

- [test/evm/solidity-source/cases/return_bool_and_or_lt_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/cases/return_bool_and_or_lt_uint_01.sol:1) 新增源码证据，源码第 5 行函数 `mixed()` 使用六个 `uint256` 参数，第 6 行返回一个 `<` 比较和两个 `<` 比较析取的 `&&`。
- [test/evm/solidity-source/ir/return_bool_and_or_lt_uint_01.ll](/sn640/NotDec/test/evm/solidity-source/ir/return_bool_and_or_lt_uint_01.ll:8) 新增冻结 IR，`public_mixed_uint256_uint256_uint256_uint256_uint256_uint256__0x2a()` 第 10-15 行用三个 `icmp ult`、一个 `or i1`、一个 `and i1` 和 `zext` 表示 bool 返回。
- [test/evm/solidity-source/expected/return_bool_and_or_lt_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/expected/return_bool_and_or_lt_uint_01.sol:1) 固化输出，第 4 行为 `return arg0 < arg1 && (arg2 < arg3 || arg4 < arg5);`。
- [test/evm/solidity-source/manifest.json](/sn640/NotDec/test/evm/solidity-source/manifest.json:297) 把新 case 接入 `notdec.evm.solidity_source`。

验证：

```bash
./llvm-22.1.0.obj/bin/llvm-as test/evm/solidity-source/ir/return_bool_and_or_lt_uint_01.ll -o /tmp/return_bool_and_or_lt_uint_01.bc
./build/bin/notdec test/evm/solidity-source/ir/return_bool_and_or_lt_uint_01.ll -o /tmp/return_bool_and_or_lt_uint_01.after.sol --tr-level=2
ctest --test-dir build -R notdec.evm.solidity_source --output-on-failure
/usr/bin/time -f 'elapsed=%e user=%U sys=%S maxrss=%M' ./build/bin/notdec test/evm/solidity-patterns/cases/25928_19774281_d048a8d52d_2758caa02f46.ll -o /tmp/notdec-solidity-bool-and-or-lt-smoke.sol --tr-level=2
```

结果：

`return_bool_and_or_lt_uint_01` 当前输出：
...
`notdec.evm.solidity_source` 通过，用时 7.17 秒。

性能：

EVM smoke 用时 `elapsed=29.85 user=33.94 sys=0.74 maxrss=981864`。

## 2026-07-01：return bool not lt uint

问题：

`return_bool_not_lt_uint_01` 的源码语义是 `return !(left < right);`。这是对比较取反的打印回归，确认当前链路能把无符号 `<` 的取反稳定输出成 `>=`。

改动：

- [test/evm/solidity-source/cases/return_bool_not_lt_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/cases/return_bool_not_lt_uint_01.sol:1) 新增源码证据，源码第 5 行函数 `notlt()` 使用两个 `uint256` 参数，第 6 行返回 `!(left < right)`。
- [test/evm/solidity-source/ir/return_bool_not_lt_uint_01.ll](/sn640/NotDec/test/evm/solidity-source/ir/return_bool_not_lt_uint_01.ll:8) 新增冻结 IR，`public_notlt_uint256_uint256__0x2a()` 第 10-12 行用 `icmp ult`、`xor i1 true` 和 `zext` 表示 bool 返回。
- [test/evm/solidity-source/expected/return_bool_not_lt_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/expected/return_bool_not_lt_uint_01.sol:1) 固化输出，第 4 行为 `return arg0 >= arg1;`。
- [test/evm/solidity-source/manifest.json](/sn640/NotDec/test/evm/solidity-source/manifest.json:279) 把新 case 接入 `notdec.evm.solidity_source`。

验证：

```bash
./llvm-22.1.0.obj/bin/llvm-as test/evm/solidity-source/ir/return_bool_not_lt_uint_01.ll -o /tmp/return_bool_not_lt_uint_01.bc
./build/bin/notdec test/evm/solidity-source/ir/return_bool_not_lt_uint_01.ll -o /tmp/return_bool_not_lt_uint_01.after.sol --tr-level=2
ctest --test-dir build -R notdec.evm.solidity_source --output-on-failure
/usr/bin/time -f 'elapsed=%e user=%U sys=%S maxrss=%M' ./build/bin/notdec test/evm/solidity-patterns/cases/25928_19774281_d048a8d52d_2758caa02f46.ll -o /tmp/notdec-solidity-bool-not-lt-smoke.sol --tr-level=2
```

结果：

`return_bool_not_lt_uint_01` 当前输出：

```solidity
contract Decompiled {
    function notlt(uint256 arg0, uint256 arg1) public returns (bool ret0) {
        // block_0:
        return arg0 >= arg1;
    }
}
...
`notdec.evm.solidity_source` 通过，用时 7.28 秒。
...
## 2026-07-01：return bool add eq uint

问题：

`return_bool_add_eq_uint_01` 的源码语义是 `return left + right == target;`。这是对算术子表达式参与 bool 比较的打印回归，确认 `add` 作为 `==` 左操作数时不会被错误加括号或丢失 bool 返回类型。这个冻结 IR 和已有 `return_uint_add_args_01` 一样只检查打印形状，不覆盖 Solidity 0.8 溢出 revert。

改动：

- [test/evm/solidity-source/cases/return_bool_add_eq_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/cases/return_bool_add_eq_uint_01.sol:1) 新增源码证据，源码第 5 行函数 `sumis()` 使用三个 `uint256` 参数，第 6 行返回加法结果与目标值的相等比较。
- [test/evm/solidity-source/ir/return_bool_add_eq_uint_01.ll](/sn640/NotDec/test/evm/solidity-source/ir/return_bool_add_eq_uint_01.ll:8) 新增冻结 IR，`public_sumis_uint256_uint256_uint256__0x2a()` 第 10-12 行用 `add`、`icmp eq` 和 `zext` 表示 bool 返回。
- [test/evm/solidity-source/expected/return_bool_add_eq_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/expected/return_bool_add_eq_uint_01.sol:1) 固化输出，第 4 行为 `return arg0 + arg1 == arg2;`。
- [test/evm/solidity-source/manifest.json](/sn640/NotDec/test/evm/solidity-source/manifest.json:243) 把新 case 接入 `notdec.evm.solidity_source`。

验证：

```bash
./llvm-22.1.0.obj/bin/llvm-as test/evm/solidity-source/ir/return_bool_add_eq_uint_01.ll -o /tmp/return_bool_add_eq_uint_01.bc
./build/bin/notdec test/evm/solidity-source/ir/return_bool_add_eq_uint_01.ll -o /tmp/return_bool_add_eq_uint_01.after.sol --tr-level=2
ctest --test-dir build -R notdec.evm.solidity_source --output-on-failure
/usr/bin/time -f 'elapsed=%e user=%U sys=%S maxrss=%M' ./build/bin/notdec test/evm/solidity-patterns/cases/25928_19774281_d048a8d52d_2758caa02f46.ll -o /tmp/notdec-solidity-bool-add-eq-smoke.sol --tr-level=2
```

结果：

`return_bool_add_eq_uint_01` 当前输出：

```solidity
contract Decompiled {
    function sumis(uint256 arg0, uint256 arg1, uint256 arg2) public returns (bool ret0) {
        // block_0:
        return arg0 + arg1 == arg2;
    }
}
...
`notdec.evm.solidity_source` 通过，用时 7.55 秒。
...
## 2026-07-01：return bool lt rhs add uint

问题：

`return_bool_lt_rhs_add_uint_01` 的源码语义是 `return left < right + extra;`。这是对比较右操作数为算术子表达式的打印回归，确认 `<` 右侧的 `add` 不会被错误加括号或拆坏优先级。这个冻结 IR 和已有加法用例一样只检查打印形状，不覆盖 Solidity 0.8 溢出 revert。

改动：

- [test/evm/solidity-source/cases/return_bool_lt_rhs_add_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/cases/return_bool_lt_rhs_add_uint_01.sol:1) 新增源码证据，源码第 5 行函数 `ltrhsadd()` 使用三个 `uint256` 参数，第 6 行返回左值和右侧加法结果的 `<` 比较。
- [test/evm/solidity-source/ir/return_bool_lt_rhs_add_uint_01.ll](/sn640/NotDec/test/evm/solidity-source/ir/return_bool_lt_rhs_add_uint_01.ll:8) 新增冻结 IR，`public_ltrhsadd_uint256_uint256_uint256__0x2a()` 第 10-12 行用 `add`、`icmp ult` 和 `zext` 表示 bool 返回。
- [test/evm/solidity-source/expected/return_bool_lt_rhs_add_uint_01.sol](/sn640/NotDec/test/evm/solidity-source/expected/return_bool_lt_rhs_add_uint_01.sol:1) 固化输出，第 4 行为 `return arg0 < arg1 + arg2;`。
- [test/evm/solidity-source/manifest.json](/sn640/NotDec/test/evm/solidity-source/manifest.json:255) 把新 case 接入 `notdec.evm.solidity_source`。

验证：

```bash
./llvm-22.1.0.obj/bin/llvm-as test/evm/solidity-source/ir/return_bool_lt_rhs_add_uint_01.ll -o /tmp/return_bool_lt_rhs_add_uint_01.bc
./build/bin/notdec test/evm/solidity-source/ir/return_bool_lt_rhs_add_uint_01.ll -o /tmp/return_bool_lt_rhs_add_uint_01.after.sol --tr-level=2
ctest --test-dir build -R notdec.evm.solidity_source --output-on-failure
/usr/bin/time -f 'elapsed=%e user=%U sys=%S maxrss=%M' ./build/bin/notdec test/evm/solidity-patterns/cases/25928_19774281_d048a8d52d_2758caa02f46.ll -o /tmp/notdec-solidity-bool-lt-rhs-add-smoke.sol --tr-level=2
```

结果：

`return_bool_lt_rhs_add_uint_01` 当前输出：

```solidity
contract Decompiled {
    function ltrhsadd(uint256 arg0, uint256 arg1, uint256 arg2) public returns (bool ret0) {
        // block_0:
        return arg0 < arg1 + arg2;
    }
}
...
`notdec.evm.solidity_source` 通过，用时 7.81 秒。
...
## 2026-07-01：return int signed modulo args

问题：

`return_int_smod_args_01` 的源码语义是 `return a % b;`，参数和返回值都是 `int256`。修复前 `BodyBuilder` 不认识 `evm_smod`，函数体退成 `return remainder;`；返回类型也还是默认 `uint256 ret0`。

改动：

- [external/NotDec-llvm2c/lib/Solidity/BodyBuilder.cpp](/sn640/NotDec/external/NotDec-llvm2c/lib/Solidity/BodyBuilder.cpp:408) 在 `evmBinaryOperatorText()` 中把 `evm_smod` 打印成 `%`。
- [external/NotDec-llvm2c/lib/Solidity/Reader.cpp](/sn640/NotDec/external/NotDec-llvm2c/lib/Solidity/Reader.cpp:51) 将上一轮的 signed return 判断改成 `isSignedArithmeticWord()`，同时识别 `evm_sdiv` 和 `evm_smod`。
- [external/NotDec-llvm2c/lib/Solidity/Reader.cpp](/sn640/NotDec/external/NotDec-llvm2c/lib/Solidity/Reader.cpp:113) 将 `returnsSingleSignedDivisionWord()` 改成 `returnsSingleSignedArithmeticWord()`，仍然只处理同一 basic block 里单 word ABI return。
- [external/NotDec-llvm2c/lib/Solidity/Reader.cpp](/sn640/NotDec/external/NotDec-llvm2c/lib/Solidity/Reader.cpp:269) 在 `Reader::readReturns()` 中复用 signed arithmetic 判断，把单 word `evm_smod` 返回类型打印成 `int256`。
- [test/evm/solidity-source/cases/return_int_smod_args_01.sol](/sn640/NotDec/test/evm/solidity-source/cases/return_int_smod_args_01.sol:1) 新增源码证据。
- [test/evm/solidity-source/ir/return_int_smod_args_01.ll](/sn640/NotDec/test/evm/solidity-source/ir/return_int_smod_args_01.ll:1) 新增冻结 IR，核心是 `evm_smod(arg0, arg1)` 后单 word return。
- [test/evm/solidity-source/expected/return_int_smod_args_01.sol](/sn640/NotDec/test/evm/solidity-source/expected/return_int_smod_args_01.sol:1) 固化输出，函数体包含 `return arg0 % arg1;`，返回类型是 `int256 ret0`。
- [test/evm/solidity-source/manifest.json](/sn640/NotDec/test/evm/solidity-source/manifest.json:63) 把新 case 接入 `notdec.evm.solidity_source`。

验证：

```bash
./llvm-22.1.0.obj/bin/llvm-as test/evm/solidity-source/ir/return_int_smod_args_01.ll -o /tmp/return_int_smod_args_01.bc
cmake --build ./build --target notdec -j4
./build/bin/notdec test/evm/solidity-source/ir/return_int_smod_args_01.ll -o /tmp/return_int_smod_args_01.after.sol --tr-level=2
ctest --test-dir build -R notdec.evm.solidity_source --output-on-failure
ctest --test-dir build -R notdec.evm.solidity_rewrite --output-on-failure
/usr/bin/time -f 'elapsed=%e user=%U sys=%S maxrss=%M' ./build/bin/notdec test/evm/solidity-patterns/cases/25928_19774281_d048a8d52d_2758caa02f46.ll -o /tmp/notdec-solidity-smod-smoke.sol --tr-level=2
```

结果：

`return_int_smod_args_01` 当前输出：

```solidity
contract Decompiled {
    function smod(int256 arg0, int256 arg1) public returns (int256 ret0) {
        // block_0:
        return arg0 % arg1;
    }
}
...
`notdec.evm.solidity_source` 通过，用时 7.96 秒；`notdec.evm.solidity_rewrite` 通过，用时 100.65 秒。

性能：

EVM smoke 用时 `elapsed=30.78 user=34.75 sys=0.76 maxrss=955824`。

## 2026-07-01：return int signed arithmetic shift right args

问题：

`return_int_sar_args_01` 的源码语义是 `return value >> shift;`，返回值是 `int256`。`evm2llvm` 对 `SAR` 的 helper 参数顺序和 `SHL/SHR` 一样是 `evm_sar(shift, value)`。修复前 `BodyBuilder` 不认识 `evm_sar`，函数体退成 `return shifted;`；返回类型也还是默认 `uint256 ret0`。

改动：

- [external/NotDec-llvm2c/lib/Solidity/BodyBuilder.cpp](/sn640/NotDec/external/NotDec-llvm2c/lib/Solidity/BodyBuilder.cpp:434) 在 `evmShiftOperatorText()` 中把 `evm_sar` 打印成 `>>`，继续复用已有 shift 参数反转打印规则。
- [external/NotDec-llvm2c/lib/Solidity/Reader.cpp](/sn640/NotDec/external/NotDec-llvm2c/lib/Solidity/Reader.cpp:51) 将 signed return helper 改成 `isSignedResultWord()`，在 `evm_sdiv`、`evm_smod` 之外加入 `evm_sar`。
- [external/NotDec-llvm2c/lib/Solidity/Reader.cpp](/sn640/NotDec/external/NotDec-llvm2c/lib/Solidity/Reader.cpp:113) 将 `returnsSingleSignedArithmeticWord()` 改成 `returnsSingleSignedResultWord()`，仍然只处理同一 basic block 里单 word ABI return。
- [external/NotDec-llvm2c/lib/Solidity/Reader.cpp](/sn640/NotDec/external/NotDec-llvm2c/lib/Solidity/Reader.cpp:269) 在 `Reader::readReturns()` 中复用 signed result 判断，把单 word `evm_sar` 返回类型打印成 `int256`。
- [test/evm/solidity-source/cases/return_int_sar_args_01.sol](/sn640/NotDec/test/evm/solidity-source/cases/return_int_sar_args_01.sol:1) 新增源码证据。
- [test/evm/solidity-source/ir/return_int_sar_args_01.ll](/sn640/NotDec/test/evm/solidity-source/ir/return_int_sar_args_01.ll:1) 新增冻结 IR，核心是 `evm_sar(arg1, arg0)` 后单 word return。
- [test/evm/solidity-source/expected/return_int_sar_args_01.sol](/sn640/NotDec/test/evm/solidity-source/expected/return_int_sar_args_01.sol:1) 固化输出，函数体包含 `return arg0 >> arg1;`，返回类型是 `int256 ret0`。
- [test/evm/solidity-source/manifest.json](/sn640/NotDec/test/evm/solidity-source/manifest.json:69) 把新 case 接入 `notdec.evm.solidity_source`。

验证：

```bash
./llvm-22.1.0.obj/bin/llvm-as test/evm/solidity-source/ir/return_int_sar_args_01.ll -o /tmp/return_int_sar_args_01.bc
cmake --build ./build --target notdec -j4
./build/bin/notdec test/evm/solidity-source/ir/return_int_sar_args_01.ll -o /tmp/return_int_sar_args_01.after.sol --tr-level=2
ctest --test-dir build -R notdec.evm.solidity_source --output-on-failure
ctest --test-dir build -R notdec.evm.solidity_rewrite --output-on-failure
/usr/bin/time -f 'elapsed=%e user=%U sys=%S maxrss=%M' ./build/bin/notdec test/evm/solidity-patterns/cases/25928_19774281_d048a8d52d_2758caa02f46.ll -o /tmp/notdec-solidity-sar-smoke.sol --tr-level=2
```

结果：

`return_int_sar_args_01` 当前输出：

```solidity
contract Decompiled {
    function sar(int256 arg0, uint256 arg1) public returns (int256 ret0) {
        // block_0:
        return arg0 >> arg1;
    }
}
...
`notdec.evm.solidity_source` 通过，用时 8.12 秒；`notdec.evm.solidity_rewrite` 通过，用时 100.30 秒。
