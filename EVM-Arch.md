...
- `SelectorEntryOutliningPass` 自己创建的 `public__notdec_solidity_selector_inline.body`
  也是 internal linkage，所以不能写成"internal 即 helper"，要和入口判定组合；
- 现有语料（`test/evm/**/*.ll`）是旧版 evm2llvm 生成的、仍然全是 external linkage，
  要让 linkage 判据在 batch 路径生效需要重新生成语料；
- 切换期间建议用 `internal || 名字以 private_ 开头` 的双条件，保证旧语料不退化。
...
1. **消费端切到 linkage**（本文第 4 节）：evm2llvm 已从生成阶段给出 internal linkage，
   把 pattern/后端的 `private__` 名字判断逐步换成双条件；语料重新生成后验证；
