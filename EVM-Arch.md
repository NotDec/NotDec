...
- **交接文档**：`logs/20260920-01-evm-calldata-record-handoff.md` —— 现状快照（提交/指标/环境）、
  多态标记的两个变体与实测数字、24259 不收敛的 profiling 证据，以及下一步（给约束引擎加周期性
  状态快照 → 判断"图在增长"还是"重复传播" → 收窄多态范围 → 打开 entry 变体并更新 oracle）。
