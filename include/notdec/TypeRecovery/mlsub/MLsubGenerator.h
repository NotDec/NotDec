...
struct TypeRecovery {
  ...
  std::shared_ptr<TRContext> TRCtx;
  ...
  TypeRecovery(std::shared_ptr<TRContext> TRCtx,
...
struct ConstraintsGenerator {
  ...
  // void removeNode(const NodeKey &K);
  ...
  CGNode &getOrCreatePrim(std::string Name, llvm::Type *LowType) {
    ...
    if (It != PrimMap.end()) {
      ...
      auto [N, NC] = CG.createNodePair(
          NodeKey(TypeVariable::CreatePrimitive(*CG.Ctx, Name), Covariant),
          LowType);
  ...
  // CGNode &getNode(const NodeKey &Key) {
  ...
  // std::shared_ptr<Sketch> solveType(const TypeVariable &Node);
  ...
  std::shared_ptr<ConstraintsGenerator>
  minimizeShared(std::map<const CGNode *, CGNode *> &Old2New);
  ...
  cloneTo(ConstraintsGenerator &Target,
          ...
          std::function<ConstraintGraph::SubtypeRelation(CGNode &, CGNode &)>
...
public:
  ...
  void addSubtype(CGNode &SubNode, CGNode &SupNode) {
    ...
    CG.addEdgeDualVariance(SubNode, SupNode, {One{}});
  }
...
inline TypeVariable getCallArgTV(TypeVariable &TV, int32_t Index) {
  return TV.pushLabel({InLabel{std::to_string(Index)}});
}

inline TypeVariable getCallRetTV(TypeVariable &TV) {
  return TV.pushLabel({OutLabel{}});
}

inline FieldLabel getCallArgLabel(int32_t Index) {
  return {InLabel{std::to_string(Index)}};
}

inline FieldLabel getCallRetLabel() { return {OutLabel{}}; }
